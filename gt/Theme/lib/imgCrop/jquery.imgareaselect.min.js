...
//        $('#s_avatar_div').empty();
        //判断图片大小
        $("#sphoto").val(img.url);
        window['a_width']=img.width;
        window['a_height']=img.height;
        var percent=1;
        if(img.width>img.height){
            if(img.width>300){
                $("#my_avatar").css({"width":"300px"});
            }
            percent=parseFloat(img.width/300);
        }else{
            if(img.height>215){
                $("#my_avatar").css({"height":"215px"});
            }
            percent=parseFloat(img.height/215);
        }
        $("#percent").val(percent);
        $("#my_avatar").attr("src",Path.web_path+"/swfupload_dir/"+img.filename+"");
        var lw=$('img#my_avatar').css('width');
        var lh=$('img#my_avatar').css('height');
        var b_img = $('#b_avatar_div').find("img");
        b_img.attr("src",Path.web_path+"/swfupload_dir/"+img.filename+"");
        setTimeout(function(){
            $('#my_avatar').imgAreaSelect({
                aspectRatio: '60:43',
                x1: 0, 
                y1: 0, 
                x2: 100, 
            ...
            setTimeout(function(){
                avatarRender.d($("#n_avatar_div img")[0],
                {
                    height:71,
                    width:100,
                    x1: 0,
     ...
     * @date 2012-12-07
     */
    d:function(img, selection){
        var bl=true;
        if (!selection.width || !selection.height)
           bl=false;
        if(!$("#my_avatar").hasClass("cancled")){
            avatarRender.e(img,selection,300,215,'#b_avatar_div img');
            $('#x').val(selection.x1);
            $('#y').val(selection.y1);
     ...
     * @date 2012-12-07
     */
    e:function(img, selection,sizeW,sizeH,e){
        var ss_width=parseInt($('#my_avatar').width(),10);
        var ss_height=parseInt($('#my_avatar').height(),10);
        var scaleX = sizeW / ((selection.width) || 1);
        var scaleY = sizeH / ((selection.height) || 1);
        var n_width = (scaleX* ss_width);
        var n_height = (scaleY * ss_height);
        $(e).css({
            width: Math.round(n_width),
