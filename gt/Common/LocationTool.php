...
class LocationTool {
	public $baidu_key = '854beda74147808fa0c20a5341e2c9ab';
	...
	public function getAddressByLocation($location_x, $location_y, $output = 'json') {
		$url = 'http://api.map.baidu.com/geocoder?output=' . $output . '&location=' . $location_x . ',' . $location_y . '&key=' . $this->baidu_key;
		return LocationTool::getResponse($url);
	}

	public function getLocationByAddress($address, $output = 'json') {
		$url = 'http://api.map.baidu.com/geocoder?output=' . $output . '&address=' . urlencode($address) . '&key=' . $this->baidu_key;
		return LocationTool::getResponse($url);
	}
	...
	/**
	 * 根据经纬度和半径获得经纬度范围
	 * @param lat 纬度 lon 经度 radius 单位米
	...
	public static function getCityByIpTaobao($ip){
		...
		$info = LocationTool::getResponse($url);
