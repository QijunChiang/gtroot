...
public class WeiXinAction extends ActionSupport {
	...
	private void getCategoryList(HttpSession session) throws Exception{
		...
		if(getCategoryTime != null){
			twoDateDifference = (int) ((new Date().getTime() - getCategoryTime.getTime())/3600000);
		}
		
		if(twoDateDifference > 24){
			String totalCategory = Util.httpUrlRequest("http://www.kaopuu.com/gtapi/app/get_category_list");
			Gson gson = new Gson();
			ResultData categoryMap = gson.fromJson(totalCategory, new TypeToken<ResultData>(){}.getType());
