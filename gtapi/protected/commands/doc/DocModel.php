...
class DocModel
{
   ...
   protected function processProperties($class)
   {
      ...
      /**
       * Add by Qijun Chiang at 2013-07-09
       */
   ...
   protected function getDocProperties($class) {
   	...
   	foreach($tags[2] as $line) {
   		...
   		$doc->definedBy = $class->getName();
   		$doc->readOnly = false;
   		$doc->isStatic = false;
   		$doc->isProtected = false;
   		$doc->isInherited = false;

   		$doc->type = array_shift($segs);
   		$doc->name = trim(array_shift($segs),"$");
   		$doc->signature = "<b>".$doc->name."</b>;";
   		$doc->signature = $this->getTypeUrl($doc->type)." ".$doc->signature;
   		/*$p->description = trim(implode(" ",$segs));*/
   		$doc->introduction = trim(implode(" ",$segs));
   		$doc->description = $doc->introduction;
