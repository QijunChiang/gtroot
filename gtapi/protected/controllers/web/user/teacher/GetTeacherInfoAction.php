 ...
 * </pre>
 * @author Qijun Chiang <QijunChiang@gmail.com>
 * @version $Id: GetTeacherInfo
 * @package com.server.controller.web.user.teacher
 * @since 0.1.0
 */
class GetTeacherInfoAction extends SessionFilterAction {
	...
	public function run() {
		...
		$data['auth']['citizenid']['id'] = '';
		$data['auth']['citizenid']['frontSide'] = '';
		$data['auth']['citizenid']['backSide'] = '';
		$data['auth']['citizenid']['status'] = '';
		$data['auth']['diploma']['id'] = '';
		$data['auth']['diploma']['diploma'] = '';
		$data['auth']['diploma']['status'] = '';
		$data['auth']['certificatesList'] = array();
		//获得用户账号信息。
		$user = User::model()
			->with(array('profile','introduction','introduction.introductionImages','teach','userAuth',
					'userAuth.userAuthCertificates','userAuth.userAuthCitizen','userAuth.userAuthDiploma',
					'userCities'
			))
			->findByPk($teacherId);
		if(!$user){
		...
		if($user_auth){
			...
			if($userAuthCitizenid){
				$data['auth']['citizenid']['id'] = $userAuthCitizenid->id;
			...
			if($userAuthDiploma){
				$data['auth']['diploma']['id'] = $userAuthDiploma->id;
			...
			foreach ($userAuthCertificates as $k=>$v){
				$certificatesList[$k]['id'] = $v->id;
				$certificatesList[$k]['image'] = $v->image;
				$certificatesList[$k]['status'] = $v->status;
			}
			$data['auth']['certificatesList'] = $certificatesList;
		}
