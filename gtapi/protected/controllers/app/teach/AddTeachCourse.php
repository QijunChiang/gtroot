 ...
 *    name: "钢琴基础入门",必选 课程名称
 *    address: "中国四川省成都市高新区软件园",必选 地址
 *    remark: "约课（时间不定，根据您和老师的时间安排）",必选 备注信息
 *    price: "80",必选 价格
 *    unit: "1",必选 价格单位，0为小时，1为课，2为总价
 *    teachTime: "5,6",必选 上课的时间，0,1,2,3,4,5,6，表示对应时间,示例：5,6 表示周六周日
 ...
 * </pre>
 * @author Qijun Chiang <QijunChiang@gmail.com>
 * @version $Id: AddTeachCourse
 * @package com.server.controller.app.teach
 * @since 0.1.0
 */
class AddTeachCourse extends SessionFilterAction {
	...
	public function run() {
		...
		$address = $this->getRequest("address",true);
		$remark = $this->getRequest("remark",true);
		$price = $this->getRequest("price",true);
		$unit = $this->getRequest("unit",true);
		$teachTime = $this->getRequest("teachTime",true);
		...
		$teachCourse = TeachCourse::model()->addTeachCourse($userId,$name,$address,$remark,
			$price,$unit,$teachTime,$usuallyLocationX,$usuallyLocationY,$address,
		...
		Notice::model()->addNotice($userId,$userId,$teachCourse->id,Contents::NOTICE_TRIGGER_TEACH_COURSE_HANDLE,Contents::NOTICE_TRIGGER_STATUS_ADD);

		$this->addResponse(Contents::RESULT,true);
		$this->addResponse(Contents::DATA,array('courseId'=>$teachCourse->id));
		$this->sendResponse();
	}
}