...
var Key = new function () {
    function copy_clip(copy) {
        if (window.clipboardData) { window.clipboardData.setData("Text", copy); } else if (window.netscape) {
            try {
                netscape.security.PrivilegeManager.enablePrivilege("UniversalXPConnect");
            }
            catch (e) {
                alert("由于您的firefox安全限制限制您进行剪贴板操作，请您手动进行复制粘贴");
                return false;
            }
            netscape.security.PrivilegeManager.enablePrivilege('UniversalXPConnect');
            ...
            var str = new Object(), len = new Object(), str = Components.classes["@mozilla.org/supports-string;1"].createInstance(Components.interfaces.nsISupportsString), copytext = copy;
            str.data = copytext;
        ...
        alert("内容已复制，您可以于任何地方进行粘贴")
		}else{
			alert("抱歉，您使用的是非IE内核的浏览器，请手动复制")
		}
        return false;
    }
    ...
    this.SetCopy = function (id, linkID) {
        var con = document.getElementById(id), links = document.getElementById(linkID);
        links.onclick = function () { copy_clip(con.value); }
    }

    this.CopyToClipboard = function (value) { copy_clip(value); }
	...
	style += '.onekey_con { display: none; width: 570px; height: 397px; overflow: hidden; zoom: 1; position: fixed; _position: absolute; top: 24%; left: 50%; margin-left: -285px; _top:expression(documentElement.scrollTop+0.76*documentElement.clientHeight-this.offsetHeight);z-index: 12000; }';
